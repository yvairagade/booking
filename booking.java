...
public class booking extends JFrame implements ActionListener {
	...
	public void paint(Graphics graf) {
		...
		switch (timechoice) {
			...
			case 3:
				for (int i = 0; i < leftside3.length; i++) {
					for (int j = 0; j < 4; j++) {
						leftside3[i][j].display(graf);
						midside3[i][j].display(graf);
						rightside3[i][j].display(graf);
					}
				}
				break;
			case 5:
				for (int i = 0; i < leftside5.length; i++) {
					for (int j = 0; j < 4; j++) {
						leftside5[i][j].display(graf);
						midside5[i][j].display(graf);
						rightside5[i][j].display(graf);
					}
				}
				break;
			case 7:
				for (int i = 0; i < leftside7.length; i++) {
					for (int j = 0; j < 4; j++) {
						leftside7[i][j].display(graf);
						midside7[i][j].display(graf);
						rightside7[i][j].display(graf);
					}
				}
				break;
			case 9:
				for (int i = 0; i < leftside9.length; i++) {
					for (int j = 0; j < 4; j++) {
						leftside9[i][j].display(graf);
						midside9[i][j].display(graf);
						rightside9[i][j].display(graf);
					}
				}
				break;
			default:
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if (ev.getSource() == paybut) {
			...
			JOptionPane.showMessageDialog(null, "These tickets have been printed out to the file 'tickets.txt'");
